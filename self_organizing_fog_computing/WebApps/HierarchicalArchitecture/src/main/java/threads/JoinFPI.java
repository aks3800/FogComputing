 ...
 *
 */
public class JoinFPI  implements Runnable {
	 ...
	 * is used as contact
	 */
	public JoinFPI(String contactNode){
	...
	public void run() {
		...
		if(contactNodeNeighbours.size()>0) {
			...
			while(contactNodeNeighbours.size()!=0 && joinGroupFlag==0) {
				...
				if(groupGraph.vertexSet().size()<=Variables.groupSize ||
					...
					Util.updateGroupGraphInNeighbours(groupGraph, contactNode, mostDistantNode);
		...
		//no group has been joined so far, which means that a new group will be created
		if(joinGroupFlag==0){
